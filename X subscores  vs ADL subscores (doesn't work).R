...
dt.tmp <- dt. %>% 
  ...
  select(
    ...
    SARA, SARA.ax, SARA.ki, s4.speech,
    # fSARA.ax, s4f.speech,
    USS
  )
...
dt.tmp2 <- bind_rows(
  dt.tmp %>% 
    select( -c(
      SARA, SARA.ki, s4.speech
    )) %>% 
    mutate( group = 'SARA.ax' ) %>% 
    rename( score = SARA.ax ) %>% 
    gather( paramcd, aval, ADL.bl, ADL.ap, ADL.ax ),
  dt.tmp %>% 
    select( -c(
      SARA, SARA.ax, s4.speech
    )) %>% 
    mutate( group = 'SARA.ki' ) %>% 
    rename( score = SARA.ki ) %>% 
    gather( paramcd, aval, ADL.bl, ADL.ap, ADL.ax ),
  dt.tmp %>% 
    select( -c(
      SARA, SARA.ax, SARA.ki, s4.speech
  ...
  mutate(paramcd = factor(paramcd, c('ADL.ax','ADL.ap','ADL.bl'))) 

dt.tmp2 %>% 
  ggplot()+geom_point()+
  aes(y = aval, x= score)+
  aes ( color = study )+ggsci::scale_color_d3(palette = "category20c")+
  facet_grid(group~paramcd)+
